...
class Records(MyWindow):
    def __init__(self, x, y, name_header_img, text_lines, font_name=FONT_NAME_RECORDS, font_size=FONT_SIZE_RECORDS, colorRGB=COLOR_MENU, activeColorRGB=COLOR_MENU_ACTIVE):
        super().__init__(x, y, name_header_img, text_lines, font_name, font_size, colorRGB, activeColorRGB)
        self.back = Text(10, WINDOW_SETTINGS[1] - self.itemH - 10, "Назад", font_name, font_size, colorRGB, activeColorRGB)
        if (len(self.items) == 0):
            self.headerW = self.back.rect.w * 1.5
            self.headerH = self.back.rect.h * 3
        else:
            ...
            for event in pg.event.get():
                ...
                if event.type == pg.MOUSEMOTION:
                    for item in self.items:
                        ...
                        if item.rect.collidepoint(event.pos):
                            item.isActive = True
                    self.back.isActive = False
                    if self.back.rect.collidepoint(event.pos):
                        self.back.isActive = True
                if event.type == pg.MOUSEBUTTONDOWN:
                    if event.button == 1:
                        if self.back.isActive:
                            self.choice = self.back.text_str
                            self.run = False
                        '''
                        for item in self.items:
                            if item.isActive:
